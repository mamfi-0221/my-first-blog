...
const Header = ({ showNav, siteTitle, scrolled, navMenuItems = [], textWhite = true }) => {
  let headerClass = "fixed w-full z-30 top-0 bg-white text-gray-800";
  headerClass += scrolled ? " bg-white shadow" : "";

  let navActionClass =
    "mx-auto lg:mx-0 hover:underline font-bold rounded-full mt-4 lg:mt-0 py-4 px-8 bg-white text-gray-800 shadow opacity-80";
  //navActionClass += !textWhite || !scrolled ? " bg-white text-gray-800" : "";
  navActionClass += scrolled ? " gradient" : "";
